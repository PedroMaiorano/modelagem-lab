# CÓPIA DE VALIDAÇÃO — NÃO É O ORIGINAL FIEL (esse está em
# docs/algoritmos-originais/Pedro_Wise_3.0.1.R e não deve ser editado).
#
# Única mudança: a linha `) %>%` seguida de `if (!is.null(mod_temp))` dentro
# de `forward_simples` (por volta da linha 197 do original) é um `%>%`
# solto/órfão — bug de digitação do autor, não um padrão de código
# intencional. Como está, o script original NÃO RODA: `source()` desse
# arquivo lança "Error in if (.) !is.null(mod_temp) else { : the condition
# has length > 1" assim que forward_simples é chamado pela primeira vez.
# Aqui removemos só o `%>%` órfão (tryCatch(...) vira uma instrução comum
# seguida do `if`, como o resto do arquivo já faz em backward_simples/
# teste_troca_simples) — nenhuma outra linha foi tocada. Usado só por
# scripts/validar_port_r.R para obter uma baseline executável do algoritmo
# original a comparar com o port Python.
